...
public class VenomTest {
    ...
    private SymbioteHost maxSuitability(SymbioteHost x, SymbioteHost y){
        if (y.calculateSuitability()<x.calculateSuitability()) {
            return x;
        }
        else{
            return y;
    ...
    public void testFindMostSuitable() {
        ...
        SymbioteHost max = test.findMostSuitable();
        SymbioteHost p1 = new SymbioteHost("Mac Gargan",47,21,false);
        SymbioteHost p2 = new SymbioteHost("Anne Weying",100,1,true);
        SymbioteHost p3 = new SymbioteHost("Wade Winston",15,5,true);
        SymbioteHost p4 = new SymbioteHost("Eddie Brock",64,24,false);
        SymbioteHost p5 = new SymbioteHost("Peter Parker",8,74,false);
        SymbioteHost p6 = new SymbioteHost("Flash Thompson",66,94,true);

        SymbioteHost sofar = maxSuitability(p1, p2);
        sofar = maxSuitability(sofar, p3);
        sofar = maxSuitability(sofar, p4);
        sofar = maxSuitability(sofar, p5);
        sofar = maxSuitability(sofar, p6);

        assertEquals(max,sofar);
    ...
    public void testFindHostsWithAntibodies() {
        ...
        ArrayList<SymbioteHost> antis = test.findHostsWithAntibodies();
        assertTrue(antis.get(0).equals(p2));
        assertTrue(antis.get(1).equals(p6));
        assertTrue(antis.get(2).equals(p3));
    ...
    public void testFindHostsWithinSuitabilityRange() {
        ...
        SymbioteHost p2 = new SymbioteHost("Eddie Brock",64,24,false);
        SymbioteHost p3 = new SymbioteHost("Peter Parker",8,74,false);
        SymbioteHost p6 = new SymbioteHost("Wade Winston",15,5,true);
        SymbioteHost p7 = new SymbioteHost("Captain America",31,5,false);
        SymbioteHost p8 = new SymbioteHost("Tony Stark",29,11,false);


        ArrayList<SymbioteHost> antis = test.findHostsWithinSuitabilityRange(70,100);
        assertTrue(antis.get(0).getName().equals(p2.getName()));
        assertTrue(antis.get(1).getName().equals(p3.getName()));
        assertTrue(antis.get(2).getName().equals(p6.getName()));
        assertTrue(antis.get(3).getName().equals(p7.getName()));
        assertTrue(antis.get(4).getName().equals(p8.getName()));
    }
    ...
    public void testDeleteSymbioteHost() {
        Venom test = new Venom();
        test.buildTree("hosts0.in");
        test.deleteSymbioteHost("Eddie Brock");
    }
